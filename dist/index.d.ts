...
import { TradeType, Currency, Percent, CurrencyAmount, Token } from '@mageswap/sdk-core';
export { Currency, TradeType } from '@mageswap/sdk-core';
import { BaseProvider, JsonRpcProvider } from '@ethersproject/providers';
export { JsonRpcProvider } from '@ethersproject/providers';
import { Trade } from '@mageswap/router-sdk';
import { ChainId } from '@mageswap/smart-order-router';
...
export { TokenInfo } from '@mageswap/token-lists';
...
type TransactionInfo = ApprovalTransactionInfo | SwapTransactionInfo | WrapTransactionInfo | UnwrapTransactionInfo;
 ...
 */
type OnSwapQuote = (args: Omit<GetQuoteArgs, 'provider' | 'onQuote'>, quote: WidgetPromise<TradeResult>) => void;
/**
...
type OnSettingsReset = () => void;
/** An integration hook called when the user changes slippage settings. */
type OnSlippageChange = (slippage: Slippage) => void;
/** An integration hook called when the user changes transaction deadline settings. */
type OnTransactionDeadlineChange = (ttl: number | undefined) => void;
type OnRouterPreferenceChange = (routerPreference: RouterPreference) => void;
...
type OnTokenChange = (field: Field, token: Currency) => void;
/**
 ...
 */
type OnAmountChange = (field: Field, amount: string, origin?: 'max') => void;
/** An integration hook called when the user switches the tokens. */
type OnSwitchTokens = () => void;
/**
 ...
 */
type OnTokenSelectorClick = (field: Field) => void | boolean | Promise<boolean>;
/** An integration hook called when the user expands a swap's details. */
type OnExpandSwapDetails = () => void;
/**
 ...
 */
type OnReviewSwapClick = () => void | boolean | Promise<boolean>;
...
type OnTxSubmit = (hash: string, tx: Transaction) => void;
type OnTxSuccess = (hash: string, tx: WithRequired<Transaction, 'receipt'>) => void;
type OnTxFail = (hash: string, receipt: TransactionReceipt) => void;
 ...
 */
type OnSwitchChain = (addChainParameter: AddEthereumChainParameter) => void | Promise<void>;

type OnConnectWalletClick = () => void | boolean | Promise<boolean>;
