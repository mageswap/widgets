import { CurrencyAmount, Price } from '@mageswap/sdk-core'
...
describe('formatTransactionAmount', () => {
  ...
  it('Number ≥ 1M extra long', () => {
    expect(formatTransactionAmount(123456789012.789)).toEqual('1.234568e+11')
  })
})
...
describe('formatDollar', () => {
  ...
  it('< 0.000001', () => {
    expect(formatDollar({ num: 0.00000000011231231432 })).toEqual('$1.12e-10')
  })
  ...
  it('lessPreciseStablecoinValues number less than 1, rounds to 0.999', () => {
    expect(formatDollar({ num: 0.9994, lessPreciseStablecoinValues: true })).toEqual('$0.999')
  })
  it('lessPreciseStablecoinValues number less than, rounds to 1.00', () => {
    expect(formatDollar({ num: 0.9995, lessPreciseStablecoinValues: true })).toEqual('$1.00')
  })
  it('lessPreciseStablecoinValues number greater than 1', () => {
    expect(formatDollar({ num: 1.0000001, lessPreciseStablecoinValues: true })).toEqual('$1.00')
  })
  it('number is greater than 1 million', () => {
    expect(formatDollar({ num: 11192312.408 })).toEqual('$1.12e+7')
  })
  it('number in the thousands', () => {
    expect(formatDollar({ num: 1234.408 })).toEqual('$1,234.41')
  })
  it('number is greater than 1.05', () => {
    expect(formatDollar({ num: 102312.408 })).toEqual('$102,312.41')
  })
})
